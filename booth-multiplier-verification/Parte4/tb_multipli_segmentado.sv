...
module tb_multipli_segmentado();
	...
	class RCSG;
		rand logic [7:0] A;
		rand logic [7:0] B;
		constraint A_pos {A[7] == 1'b0;}
		constraint A_neg {A[7] == 1'b1;}
		constraint B_pos {B[7] == 1'b0;}
		constraint B_neg {B[7] == 1'b1;}
	endclass
...
// Comprobamos que la multiplicacion se realiza de manera correcta
	assert property (@(posedge CLOCK) END_MULT |=> ($signed(S) == $signed(A)*$signed(B)))	
	else	$error("El multiplicador no realiza su funcion %d,%d,%d",A,B,S);
